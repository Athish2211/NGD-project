pipeline {
  ...
  parameters {
    ...
    string(name: 'PRODUCTS_DATASET_FILE', defaultValue: 'server/scripts/datasets/scraped_pricehistory.json', description: 'Dataset file path inside the repo.')
  ...
  stages {
    ...
    stage('Deploy to NGD Server') {
      agent any
      steps {
        echo 'Deploying to NGD Application/Server for DevOps Demo'
        sh '''
          # Simulating a deployment process (e.g., SSH/SCP or RSYNC)
          echo "Connecting to NGD server..."
          echo "Copying latest build artifacts..."
          echo "Restarting NGD services..."
